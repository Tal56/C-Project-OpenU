...
void read_line(char *line)
{
    ...
    if(is_label(current_token, COLON)) /* checks if the first token is a label*/
	{ 
        label = TRUE;
    ...
    if((dir_type = find_directive(current_token)) != NOT_FOUND) /* detecting directive type (if it's a directive) */
    {
        if(label)
        {
            if(dir_type == EXTERN || dir_type == ENTRY) /* we need to ignore creation of label before .entry or .extern */
			{ 
...
int handle_data_directive(char *line)
{
    ...
    while(!end_of_line(line))
    {
        ...
        if(strlen(token) > 0) 
        {
            ...
            else /* If there was a comma, it should be only once */
            {
                if(comma) 
				{
...
void write_string_to_data(char *str)
{
    ...
    {
        data[dc++] = (unsigned int) *str; /* Inserts a character to data array */
        str++;
    }
    data[dc++] = '\0';
}
...
boolean is_label(char *token, int colon)
{
    ...
    if(colon && token[token_len - 1] != ':') /* if colon = TRUE, there must be a colon at the end */
		return FALSE; 
